        ...
        for attempt_number in range(0, MAX_BROADCAST_ATTEMPTS):
            for m in get_providers_for_chain(self.ethereum_chain, self.local_node):
                try:
                    ...
                    if (txid):
                        logging.info(
                            'Broadcasting succeeded with method_provider=%s, txid=%s', str(m), txid)
...
class EtherscanBroadcaster(object):
    ...
    def broadcast_tx(self, tx, api_token):
        ...
        if 'error' in response.json():
            logging.error("Etherscan returned an error: %s",
                          response.json()['error'])
            raise BroadcastError(response.json()['error'])
        if int(response.status_code) == 200:
            tx_id = response.json().get('result', None)
            logging.info(
                "Transaction ID obtained from broadcast through Etherscan: %s", tx_id)
            return tx_id
        logging.error(
            'Error broadcasting the transaction through the Etherscan API. Error msg: %s', response.text)
    ...
    def get_address_nonce(self, address, api_token):
        ...
        raise BroadcastError(
            'Error checking the nonce through the Etherscan API. Error msg: %s', response.text)
