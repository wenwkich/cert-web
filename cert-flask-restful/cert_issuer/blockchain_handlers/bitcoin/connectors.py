...
class BlockExplorerBroadcaster(object):
    ...
    def broadcast_tx(self, tx):
        ...
        if int(response.status_code) == 200:
            ...
            return tx_id
        logging.error(
            'Error broadcasting the transaction through the BlockExplorer API. Error msg: %s', response.text)
...
class BlockcypherProvider(object):
    ...
    def broadcast_tx(self, tx):
        ...
        if int(response.status_code) == 201:
            ...
            return tx_hash
        logging.error(
            'Error broadcasting the transaction through the Blockcypher API. Error msg: %s', response.text)
        raise BroadcastError(response.text)
...
class BlockstreamBroadcaster(object):
    ...
    def broadcast_tx(self, tx):
        ...
        if int(response.status_code) == 200:
            ...
            return tx_id
        logging.error(
            'Error broadcasting the transaction through the Blockstream API. Error msg: %s', response.text)
        raise BroadcastError(response.text)
...
class BitpayBroadcaster(object):
    ...
    def broadcast_tx(self, tx):
        ...
        if int(response.status_code) == 200:
            ...
            return tx_id
        logging.error(
            'Error broadcasting the transaction through the Bitpay API. Error msg: %s', response.text)
        raise BroadcastError(response.text)
...
class BitcoindConnector(object):
    ...
    def spendables_for_address(self, address):
        ...
        for unspent in unspent_outputs:
            ...
            spendables.append(Spendable(coin_value, script,
...
provider_list.append(BlockcypherProvider(
    'https://api.blockcypher.com/v1/btc/main', blockcypher_token))
provider_list.append(InsightProvider(
    netcode=helpers.to_pycoin_chain(Chain.bitcoin_mainnet)))
provider_list.append(ChainSoProvider(
    netcode=helpers.to_pycoin_chain(Chain.bitcoin_mainnet)))
...
xtn_provider_list.append(ChainSoProvider(
    netcode=helpers.to_pycoin_chain(Chain.bitcoin_testnet)))
xtn_provider_list.append(BlockcypherProvider(
    'https://api.blockcypher.com/v1/btc/test3', blockcypher_token))
xtn_provider_list.append(BlockExplorerBroadcaster(
    'https://testnet.blockexplorer.com/api'))
xtn_provider_list.append(BlockstreamBroadcaster(
    'https://blockstream.info/testnet/api'))
xtn_provider_list.append(BitpayBroadcaster(
